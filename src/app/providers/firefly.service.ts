...
export class FireflyService implements BaseInterface{
  ...
  public async getEntities(endpoint: string, recursive: boolean = false) {
    return new Promise((resolve, reject) => {
      this.getHttpHeaders()
        .then(h => { 
  ...
  public getEntitiesRecursive(url, headers, entities: any, resolve, reject, recurse){
